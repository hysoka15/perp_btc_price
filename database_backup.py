        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                for row in raw_data:
                    ...
                    # 计算时间桶
                    bucket = (timestamp_unix // interval_seconds) * interval_seconds
                    bucket_time = datetime.fromtimestamp(bucket).strftime('%Y-%m-%d %H:%M:%S')
                    
                    if bucket_time not in groups:
                        groups[bucket_time] = []
                    
                    groups[bucket_time].append({
                        'price_diff': row['price_diff'],
                        'price': row['price'],
                        'binance_base_price': row['binance_base_price'],
                        'timestamp': row['timestamp']
                    })
                
                # 生成K线数据
                kline_data = []
                for bucket_time in sorted(groups.keys(), reverse=True)[:limit]:
                    group = groups[bucket_time]
                    if not group:
                        continue
                    
                    price_diffs = [item['price_diff'] for item in group if item['price_diff'] is not None]
                    if not price_diffs:
                        continue
                    ...
                    kline_data.append({
                        ...
                        'open': price_diffs[0],
                        'high': max(price_diffs),
                        'low': min(price_diffs),
                        'close': price_diffs[-1],
                        'volume': len(price_diffs),
                        'avg_price': sum(item['price'] for item in group) / len(group),
                        'avg_binance_price': sum(item['binance_base_price'] or 0 for item in group) / len(group)
                    })
                
                return kline_data
                
        except Exception as e:
            logger.error(f"获取简单K线数据失败 {exchange} {interval}: {str(e)}")
            return []
